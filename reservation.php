  ...
  if (isset($_GET['id'])) {
    ...
    if (isset($_POST['submit'])) {
      ...
      if (
        empty($_POST['name']) or empty($_POST['phone_number']) or empty($_POST['num_of_geusts'])
        ...
        if (date("Y-m-d") < $checkin_date) {
          ...
          $insert = $conn->prepare("INSERT INTO bookings (name, phone_number, num_of_geusts, checkin_date, destination, status, city_id, user_id, payment)
      VALUES (:name, :phone_number, :num_of_geusts, :checkin_date, :destination, :status, :city_id, :user_id, :payment)");
          ...
          echo "<script>alert('Invalid Date, Pick starting from tommoro');</script>";
        }
      }
    }
  } else {
